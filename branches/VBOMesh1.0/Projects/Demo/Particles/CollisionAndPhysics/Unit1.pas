...
uses
  ...
  PFXManager, ComCtrls, ExtCtrls, Math, TGA;
type
  TForm1 = class(TForm)
    GLScene1: TGLScene;
...
begin
  ...
  with GLMaterialLibrary1.AddTextureMaterial('Back','..\Media\3D_nature_048.jpg').Material
  do begin
  ...
  with Meshes.AddScreenQuad do begin
    Material:=GLMaterialLibrary1.Materials[2].Material;
    NoZWrite:=true;
    visible:=false;
  end;
  ...
  Particles:=Meshes.AddParticles(10000).AsVBOParticles;
  with Particles do begin
     Material:=GLMaterialLibrary1.LibMaterialByName('Snow').Material;
     Name:='Particles';
       ...
       //”меньшение прозрачности по мере удалени€ от камеры
        if CheckBox5.Checked then begin
          ...
          if c1[3]<>d then begin
            c1[3]:=d; Particles.Colors[PIndex]:=c1;
          end;
        end;
      end;
   end;
...
begin
  Meshes[2].AsVBOParticles.Material.Texture.Disabled:=not CheckBox3.Checked;
end;
