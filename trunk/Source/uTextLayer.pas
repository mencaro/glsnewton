...
Type
  TTextureFont = class
   private
     ...
     FBitmap: TBitmap;
     FTextureFile: TTexture;
     FCreationType: (fBuilded, fLoaded);
   public
     constructor Create;
     destructor Destroy;override;
     procedure Assign(Font: TTextureFont);
  ...
  TTextSurface = class (TVBOMeshObject)
    private
      ...
      function FInitFBO(w,h: integer): TTexture;
    ...
    public
      ...
      property TextTexture: TTexture read FFBOTex;
  ...
  TVBOTextRect = class (TVBOMeshObject)
    ...
    public
      ...
      destructor Destroy; override;
      procedure RenderObject(const ViewMatrix: TMatrix);override;
      procedure SetFont(FontName: string; FontSize: integer; Style: TFontStyles=[]);
...
function TTextSurface.SetText(Text: TStringList; col:integer=-1; row:integer=-1): GLUint;
  ...
  function TTextSurface.FInitFBO(w,h: integer): TTexture;
  var fbo_tex: TTexture;
  begin
...
procedure TTextSurface.FApplyShader(mo: TObject);
  var v:TVector;
  begin
  ...
  procedure TTextSurface.FUnApplyShader(mo: TObject);
  begin
     if (not FBO.Active) or (not assigned(FShaders)) then exit;
     glActiveTexture(GL_TEXTURE1);
...
constructor TTextSurface.Create;
  ...
  procedure TTextSurface.RenderObject(const ViewMatrix: TMatrix);
  begin
...
procedure TVBOTextRect.RenderObject(const ViewMatrix: TMatrix);
var m: TMatrix;
...
procedure TVBOTextRect.SetFont(FontName: string; FontSize: integer;
  Style: TFontStyles);
begin
  Font.BuildTexture(FontName,FontSize,Style);
end;
...
procedure TTextureFont.Assign(Font: TTextureFont);
begin
  FCharsFWidth:=Font.FCharsFWidth;
...
var i: integer;
begin
  FTextureFile:=TTexture.CreateFromFile(Filename,ttTextureRectangle);
