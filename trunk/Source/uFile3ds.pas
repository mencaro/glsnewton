...
uses classes, File3DS, Types3DS, VectorGeometry,
   {$IFNDEF DIRECTGL}
     OpenGL1x, VectorLists,
   {$ELSE}
     dglOpenGL, uVectorLists,
   {$ENDIF}
     SysUtils, uMeshObjects, uVBO, uMiscUtils, uBaseClasses,
     uMaterials, uTextures, uMaterialObjects, uKeyFrames, OGLStateEmul;
Type
  THashArray = class
  private
    FCount: integer;
...
begin
  ...
  //–езервируем место под массивы вершин
  ...
  //«аполн€ем массивы вершин согласно материалам
  for i:=0 to obj.MeshCount-1 do begin
  ...
  //»ндексируем вершины и формируем буфер VBO
  for i:=0 to high(MHT) do begin
...
//-------------------------
//-------—оздаем материалы и загружаем текстуры
//-------------------------
...
begin
  ...
  dt:=FTime-fStartTime; //врем€ в секундах
  KeyFrames:=FKeyFrameList[Index];
  FrameTime:=1/KeyFrames.FrameRate;
  FrameN:=dt/FrameTime;
  if FrameN>=KeyFrames.Count then begin
    if FPlayLoop then FrameN:=FrameN-trunc(FrameN/KeyFrames.Count)*KeyFrames.Count
    else FrameN:=KeyFrames.Count-1;
  end;
