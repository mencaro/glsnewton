...
begin
  ...
  with world.AddScreenQuad do begin
    Texture:=fbotex2; //Текстура сцены с позиции основной камеры
  end;

  with world.AddHUDSprite(0.5,0.5) do begin
    Texture:=dpt2; //Текстура глубины сцены с позиции основной камеры
  ...
  with world.AddHUDSprite(0.5,0.5) do begin
    Texture:=dpt2; //линеризиванная глубина сцены с основной камеры
    Shader:=DVShader;
    MoveObject(-0.75,+0.25,0);
  end;

  with world.AddHUDSprite(0.5,0.5) do begin
    Texture:=depthtex; //текстура глубины сцены с позиции источника света
    MoveObject(-0.75,-0.25,0);
  end;
